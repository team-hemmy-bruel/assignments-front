...
export class AuthBdl {
  ...
  register(email: string, password: string, username: string, status: string): Observable<boolean> {
    return new Observable((observer) => {
      this.auhtSA.register(email, password, username, status).subscribe(data => {
        if (data.auth) {
          this.storageService.setToken(data.token);
          this.userBdl.me();
          console.log('connected :>> ');
          ...
          console.log('some error ocrrurs :>> ');
